...
class VideoFrames():
    ...
    def build_lists(self, video_start_time_utc):
        ...
        print 'Building image list: found duration: ', duration
        count = 0
        ...
        while count < self.frame_count:
            ...
            print 'Total time UTC: ', video_start_time_utc + datetime.timedelta(seconds=(1/self.fps)*count)
            total_utc_time = video_start_time_utc + datetime.timedelta(seconds=elapsed_time)
            self.utc_list.append(total_utc_time)
            self.epoch_list.append(convert_utc_to_epoch(total_utc_time))
